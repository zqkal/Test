...
/*********************************************************************************/
/*                                                                               */
/*  Macro: PRD04_get_assay_results.sas                                           */
/*                                                                               */
...
/*                                                                               */
/*  Usage: %PRD04_get_assay_results(network=test, protocol=999,                  */
...
%macro PRD04_get_assay_results(network=, protocol=, file_name_orig=, file_name_clean=,
   ...
   %else %do;
      ...
      %check_header(header = STUDY_ID PTID GUSPEC COLLDT VISIT TIME/UNIT ASSAY_DATE BATCH_ID DRUG CONC UNITS CENSORS COMMENTS,
                    file = &file.,
                    header_row = 1,
                    checkremainder = 1,
      ...
      data &out_ds_all. (drop=infile_value)
         ...
         /* Read in the file */
         /*------------------*/
         input prot ptidc guspec1 spcdtc visitnoc pktime assaydtc runid drug concc concunit censor comments1 blank_col;
         ...
         %input_ptid(char_var=ptidc, num_var=ptid)
      ...
      %add_miss_value(miss_dataset=&out_ds_all.,
                      miss_vars=primstr addstr dervst2 sec_typ sec_id spctm volume volstr,
                      search_vars=guspec,
